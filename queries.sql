...
BEFORE UPDATE ON SURVEYS
FOR EACH ROW
WHEN (OLD.UPDATED_AT IS DISTINCT FROM NEW.UPDATED_AT AND OLD.CATEGORY = 1 AND NEW.CATEGORY = 1)
...
/*
 * Создать процедуру для создания актуальных (динамичных) данных в таблице "SURVEYS_EMPLOYEES_RELATIONSHIP" (old version).
 */
...
/*
 * Удалить процедуру "creator" (old version).
 */
DROP PROCEDURE IF EXISTS creator(uuid, varchar[]);

/*
 * Вызов процедуры "creator" (old version).
 */
CALL creator('99c89a24-fff2-4cbc-a542-b1e956a352f9', ARRAY['NNogerbek@beeline.kz']);

/*
 * Создать процедуру для создания актуальных (динамичных) данных в таблице "SURVEYS_EMPLOYEES_RELATIONSHIP" (new version).
 */
...
/*
 * Удалить процедуру "creator" (new version).
 */
DROP PROCEDURE IF EXISTS creator(uuid, VARCHAR[], VARCHAR[]);

/*
 * Вызов процедуры "creator" (new version).
 */
CALL creator(
  '99c89a24-fff2-4cbc-a542-b1e956a352f9',
  ARRAY['NNogerbek@beeline.kz', 'VChervonnaya@beeline.kz'],
  ARRAY['\27623\30553\28134\30503\30514\28274', '\27623\30553\28134\30502\30510\28380']
);
...
BEGIN
  DELETE FROM QUESTIONS_OPTIONS_RELATIONSHIP WHERE QUESTION_ID IN (
...
/*
 * Удалить constraint "ORGANIZATIONAL_STRUCTURE_HISTORY_UNIQUE_KEY" из таблицы "ORGANIZATIONAL_STRUCTURE_HISTORY".
 */
ALTER TABLE ORGANIZATIONAL_STRUCTURE_HISTORY DROP CONSTRAINT ORGANIZATIONAL_STRUCTURE_HISTORY_UNIQUE_KEY;

/*
 * Добавить новый столбец "TREE_ORGANIZATION_ID" в таблицу "SURVEYS_EMPLOYEES_RELATIONSHIP".
 */
ALTER TABLE SURVEYS_EMPLOYEES_RELATIONSHIP ADD COLUMN TREE_ORGANIZATION_ID VARCHAR;

/*
 * Удалить столбец "TREE_ORGANIZATION_ID" из таблицы "SURVEYS_EMPLOYEES_RELATIONSHIP".
 */
ALTER TABLE SURVEYS_EMPLOYEES_RELATIONSHIP DROP COLUMN TREE_ORGANIZATION_ID;