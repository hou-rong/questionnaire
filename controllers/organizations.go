...
var GetOrganizations = func(responseWriter http.ResponseWriter, request *http.Request) {
	...
	if len(keys) != 0 {
		...
		if len(surveyIdentifier) != 0 {
			rows, err := database.DBSQL.Query("SELECT ORGANIZATION_ID, ORGANIZATION_NAME, ORGANIZATION_RANG, PARENT_ORGANIZATION_ID FROM ORGANIZATIONAL_STRUCTURE_HISTORY WHERE ORGANIZATION_NAME IS NOT NULL AND RANG1_ORGANIZATION_ID NOT IN (28825, 27624, 27626, 28833, 29033) AND SURVEY_ID = $1 ORDER BY ORGANIZATION_RANG, ORGANIZATION_ID;", surveyIdentifier); if err != nil {
				logger.Println(err)
		...
		rows, err := database.OracleDB.Query(`SELECT DISTINCT 
        	O.ORGANIZATION_ID,
            O.ORGANIZATION_NAME,
            O.ORGANIZATION_RANG,
