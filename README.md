...
FOR EACH ROW
WHEN (
    OLD.UPDATED_AT IS DISTINCT FROM NEW.UPDATED_AT AND OLD.CATEGORY = 1 AND NEW.CATEGORY = 1
...
`creator` procedure:
```
CREATE OR REPLACE PROCEDURE creator(SURVEY_IDENTIFIER uuid, EMPLOYEES VARCHAR[], TREE_ORGANIZATION_IDS VARCHAR[]) AS $FUNCTION$
BEGIN
    DELETE FROM SURVEYS_EMPLOYEES_RELATIONSHIP
    WHERE SURVEY_ID = SURVEY_IDENTIFIER
    AND EMPLOYEE <> ALL (EMPLOYEES);
    INSERT INTO SURVEYS_EMPLOYEES_RELATIONSHIP (SURVEY_ID, EMPLOYEE, TREE_ORGANIZATION_ID)
    SELECT DISTINCT ON (u.EMPLOYEE) SURVEY_IDENTIFIER AS SURVEY_ID, u.EMPLOYEE, u.TREE_ORGANIZATION_ID FROM UNNEST(ARRAY[EMPLOYEES], ARRAY[TREE_ORGANIZATION_IDS]) u(EMPLOYEE, TREE_ORGANIZATION_ID)
    ORDER BY u.EMPLOYEE, u DESC
    ...
    WHERE NOT SURVEYS_EMPLOYEES_RELATIONSHIP.STATUS;
END;
$FUNCTION$ LANGUAGE plpgsql;
```
